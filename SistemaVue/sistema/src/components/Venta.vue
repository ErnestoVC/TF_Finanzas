        ...
        <v-flex>
            ...
            >
                <template slot="items" slot-scope="props">
                    ...
                    <td>{{ props.item.procen_interes}}</td>
                    <td>{{ props.item.interes}}</td>
                    <td>{{ props.item.total_venta}}</td>
                    <td>{{ props.item.total_deuda}}</td>
                    <td>{{ props.item.tipo_interes}}</td>
                    <td>
            ...
            <v-container grid-list-sm class="pa-4 white" v-if="verNuevo">
                <v-layout row wrap>
                    <v-flex xs12 sm4 md4 lg4 xl4>
                        <v-select v-model="tipo_interes" 
                        :items="tasas" label="Tipo Tasas">
                        </v-select>
                    </v-flex>
                    ...
                    <v-flex xs12 sm4 md4 lg4 xl4>
                        <v-text-field type="number" v-model="procen_interes" label="Porcentaje Interes">
                        </v-text-field>
                    </v-flex>
                    <v-flex xs12 sm4 md4 lg4 xl4>
                        <v-text-field type="number" v-model="dias" label="Dias">
                        </v-text-field>
                    </v-flex>
                    <v-flex xs12 sm8 md8 lg8 xl8>
                    ...
                    <v-flex xs12 sm12 md12 lg12 xl12>
                        ...
                        <v-flex class="text-xs-right">
                            <strong>Total Venta: </strong>$ {{total_venta=(calcularTotal).toFixed(2)}}
                        </v-flex>
                        <v-flex class="text-xs-right">
                            <strong>Interes: </strong>$ {{interes=total_deuda-total_venta}}
                        </v-flex>
                        <v-flex class="text-xs-right">
                            <strong>Total Deuda: </strong>$ {{total_deuda= calcularDeuda ? calcularDeuda.toFixed(2) : null}}
                        </v-flex>
...
<script>
    ...
    export default {
        data(){
            return {
                ...
                headers: [
                    ...
                    { text: 'Tipo Tasas', value: 'tipo_interes' },
                    { text: 'Porcentaje Interes', value: 'procen_interes' },
                    { text: 'Días', value: 'dias' },
                    { text: 'Intereses', value: 'intereses' },
                    { text: 'Total Venta', value: 'total_venta' },
                    { text: 'Total Deuda', value: 'total_deuda' },
                ...
                tipo_interes: '',
                tasas: ['TASA SIMPLE','TASA NOMINAL ANUAL','TASA EFECTIVA ANUAL'],
                procen_interes: 8,
                codigo:'',
                verNuevo:0,
                errorArticulo:null,
        ...
        computed: {
            ...
            calcularDeuda:function(){
                var capital = this.calcularTotal;
                var m = 12;
                var stock = 0.0;
                switch (this.tipo_interes) {
                    case 'TASA SIMPLE':
                        stock = capital*(1+(this.procen_interes/100)*(this.dias/360))
                        return stock;
                        break;
                    case 'TASA NOMINAL ANUAL':
                        stock=capital*(Math.pow((1+((this.procen_interes/100)/m)),this.dias))
                        return stock;
                        break;
                    case 'TASA EFECTIVA ANUAL':
                        stock=capital*Math.pow((1+(this.procen_interes/100)),(this.dias/360))
                    default:
                        break;
                }
            }
        ...
        methods:{
            ...
            mostrarComprobante(item){
                this.limpiar();
                this.dias=item.dias;
                this.procen_interes = item.procen_interes;
                this.interes = item.interes;
                this.total_venta = item.total_venta;
                this.total_deuda = item.total_deuda;
            ...
            guardar () {
                ...
                axios.post('api/Ventas/Crear',{
                    ...
                    'dias': me.dias,
                    'procen_interes': me.procen_interes,
                    'intereses': me.interes,
                    'total_venta': me.total_venta,
                    'total_deuda': me.total_deuda,
                    'tipo_interes': me.tipo_interes,
            ...
            validar(){
                ...
                if (!this.tipo_interes){
                    this.validaMensaje.push("Seleccione un tipo de interes.");
                }
